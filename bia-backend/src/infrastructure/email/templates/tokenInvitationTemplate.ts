/**
 * Email template for process-level token invitations.
 * Validates: Requirements 7.1, 7.2
 */
export function tokenInvitationTemplate(data: {
  areaName: string;
  processoName: string;
  link: string;
}): string {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
      ...
      <p style="color: #444; line-height: 1.6;">
        Você foi convidado para avaliar o processo <strong>${data.processoName}</strong> da área <strong>${data.areaName}</strong>.
      </p>
